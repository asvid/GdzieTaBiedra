...
public class Database {
    ...
    private static Realm realm = App.getRealm();

    public static void populate(Double lat, Double lng) {
        App.shopApi.getShops(lat, lng, new Callback<ShopList>() {
            @Override
            public void success(ShopList shopList, Response response) {
                ...
                EventBus.getDefault().post(new MessageEvent("new data",
    ...
    public static List<Shop> getListClosest() {
        ...
        Location lastLocation = ((LocationManager) App.getAppContext()
        ...
        if (lastLocation != null) {
            for (int i = 0, l = query.size(); i < l; i++) {
                ...
                currentLoc
                        .setLatitude(Double.parseDouble(current.getLatitude()));
                currentLoc.setLongitude(
                        Double.parseDouble(current.getLongitude()));
                if (lastLocation.distanceTo(currentLoc) < Const.radius) {
                    realm.beginTransaction();
                    current.setDistance(
                            lastLocation.distanceTo(currentLoc) + "");
                    realm.commitTransaction();
        ...
        Collections.sort(result, new Comparator<Shop>() {
            ...
            public int compare(Shop lhs, Shop rhs) {
                if (Double.parseDouble(lhs.getDistance()) == Double
                        .parseDouble(rhs.getDistance()))
                    return 0;
                return Double.parseDouble(lhs.getDistance()) < Double
                        .parseDouble(rhs.getDistance()) ? -1 : 1;
            }
        });
        return result;
    }
